...
export default function App() {
  ...
  const displayedSimulationTime = simulation.mode === 'collapse'
    ? (simulation.isRunning ? simulation.simulationTime : simulation.startDate)
    : (simulation.isRunning ? simulation.simClock : simulation.startDate);

  const [realClock, setRealClock] = useState(() => new Date());

  React.useEffect(() => {
    if (!simulation.isRunning && simulation.mode !== '5day') return;
    const id = setInterval(() => setRealClock(new Date()), 1000);
    return () => clearInterval(id);
  }, [simulation.isRunning, simulation.mode]);

  const simClockDisplay = useMemo(() => {
    const months = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'];
    const d = displayedSimulationTime;
    const running = simulation.isRunning;
    const dd = String(running ? d.getUTCDate() : d.getDate()).padStart(2, '0');
    const mon = months[running ? d.getUTCMonth() : d.getMonth()];
    const hh = String(running ? d.getUTCHours() : d.getHours()).padStart(2, '0');
    const mi = String(running ? d.getUTCMinutes() : d.getMinutes()).padStart(2, '0');
    const ss = String(running ? d.getUTCSeconds() : d.getSeconds()).padStart(2, '0');
    return { date: `${dd} ${mon}`, time: `${hh}:${mi}:${ss}` };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden relative">
            ...
            {(simulation.isRunning || simulation.simulationComplete || simulation.collapseComplete) && (
              <div style={{
