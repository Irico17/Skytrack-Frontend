...
export function StaticDataUploadModal({ onClose, onUpload }: StaticDataUploadModalProps) {
  ...
  const totalBatches = Math.max(1, Math.ceil(shipmentFiles.length / 10));
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="mt-1.5 flex items-center gap-1.5 text-[10px] text-[#6080A0]">
              ...
              {shipmentFiles.length > 0
                ? `${shipmentFiles.length} archivos (${totalBatches} lotes de hasta 10)`
                : 'Sin archivos'}
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-3 rounded-xl bg-[#FFC857]/8 border border-[#FFC857]/20">
            ...
            <div className="text-[11px] text-[#7088A8] leading-relaxed">
              Los envíos se suben en lotes de 10. El backend valida todo en staging y solo reemplaza los datos activos al finalizar.
            </div>
          </div>
